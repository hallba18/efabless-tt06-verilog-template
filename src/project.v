...
\TLV calc()
   |encrypt
      @0
         ...
         $ui_in[7:0] = 8'b1000_0000;   //Input to determine mode/keys
         $ofb = $ui_in[7];             //Switch to determine mode
         $blocks_to_run[22:0] = 2;     //Blocks of AES to run if in OFB
         ...
         ?$valid_blk
            ...
            $ld_key = ((!$reset && >>1$reset) || $r_counter == 11) ? 1 : 0;
            
            $run_key = (!$ld_key && >>1$ld_key) ? 1 :
                       (>>1$run_key && $ofb && >>1$reset_count <= $blocks_to_run ) ? 1 :
